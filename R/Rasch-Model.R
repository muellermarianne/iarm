...
#'
#' @param x vector of levels m_i for items i=1 to k.
...
#' @param  obj object of class "eRm" (but not "dRm")
...
clr_tests <- function(dat.items, dat.exo, model = c("RM","PCM")) {
  ...
  if (model == "RM") {
    ...
    clr0 <- -2*(PCM(dat.i)$loglik - (PCM(dat.i[sgrp==1, ])$loglik + PCM(dat.i[sgrp==2, ])$loglik))
    clrhomo <- function(exo, data){
      sum(sapply(split(data, exo, drop = TRUE),function(x){PCM(x)$loglik}))
    }
    ll <- PCM(dat.items[ok1, ])$loglik
    m <- sum(apply(dat.items[ok1, ], 2, max, na.rm = TRUE))
  }
  clr <- c(clr0, -2*(ll - apply(dat.exo[ok1, ],2,clrhomo, dat.items[ok1, ])))
  df <- (m-1)* (c(1, sapply(dat.exo[ok1, ],function(x){sum(table(x) != 0)}) - 1))
  mm <- cbind(clr, df)
  row.names(mm) <- c("overall", names(dat.exo[ok1, ]))
